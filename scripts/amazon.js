  ...
  if (search) {
    filteredProducts = products.filter((product) => {
      ...
      product.keywords.forEach((keyword) => {
        if (keyword.toLowerCase().includes(search.toLowerCase().replace(/[^a-zA-Z0-9]/g, ''))) {
          matchingKeyword = true;
        }
      });
      
      return product.name.toLowerCase().includes(search.toLowerCase().replace(/[^a-zA-Z0-9]/g, '')) || matchingKeyword;
    });
  }
